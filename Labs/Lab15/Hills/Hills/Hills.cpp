...
void Hills::BuildGeometry()
{
    ...
    graphics->Copy(grid.VertexData(), vbSize, geometry->vertexBufferCPU);
    graphics->Copy(grid.IndexData(), ibSize, geometry->indexBufferCPU);

    // copia vértices e índices para a GPU usando o buffer de Upload
    graphics->Copy(grid.VertexData(), vbSize, geometry->vertexBufferUpload, geometry->vertexBufferGPU);
    graphics->Copy(grid.IndexData(), ibSize, geometry->indexBufferUpload, geometry->indexBufferGPU);
}
